...
The CPP provides a principled framework for answering these questions. In addition, we model the \emph{continuous} component of volatility dynamics using a mean--variance decomposition on $d_t=\Delta\log(\mathrm{VIX})_t$: an ARIMA model for the conditional mean and GARCH-family models (GARCH/EGARCH) for the conditional variance. This volatility modeling complements the CPP shock framework by capturing volatility clustering and asymmetry, and it enables a concise persistence metric via the \textbf{volatility half-life} (how quickly volatility shocks decay back toward baseline).
...
Let $\varepsilon_t$ denote innovations. An ARIMA$(p,0,q)$ (ARMA$(p,q)$) model takes the form
\[
d_t = c + \sum_{i=1}^{p}\phi_i d_{t-i} + \varepsilon_t + \sum_{j=1}^{q}\theta_j \varepsilon_{t-j}.
\]

\subsubsection{GARCH$(p,q)$ conditional variance}
A GARCH$(p,q)$ model specifies time-varying conditional variance via
\[
...
EGARCH models log-variance and allow asymmetric responses. For EGARCH$(1,1)$, letting $z_{t-1}=\varepsilon_{t-1}/\sigma_{t-1}$,
\[
\log(\sigma_t^2)
=
\omega
...
We model $d_t=\Delta\log(\mathrm{VIX})_t$ and select the ARIMA order by information criteria (BIC). The chosen specification is ARIMA$(1,0,1)$ (ARMA$(1,1)$).

\subsubsection{Volatility model selection (GARCH/EGARCH)}
We fit GARCH-family models on the ARMA residuals and compare variants by AIC/BIC across (i) volatility specification (GARCH vs.\ EGARCH), (ii) orders $(p,q)\in\{1,2\}\times\{1,2\}$, and (iii) innovation distribution (normal vs.\ Student-\emph{t}). Table~\ref{tab:garch_grid} reports the comparison (sorted by BIC).
...
\subsubsection{Jump size distributions}
...
We fit GARCH-family models to the ARMA residuals $\varepsilon_t$ with both normal and Student-\emph{t} innovations. By BIC, the best volatility specification is \textbf{EGARCH$(1,1)$ with Student-\emph{t} innovations}.
