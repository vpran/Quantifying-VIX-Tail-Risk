...
%  Title
% ============================================================
\title{\textbf{Compound Poisson Process for VIX Shock Modeling}}
...
Traditional point process models for financial shocks---such as the Homogeneous Poisson Process (HPP), Non-Homogeneous Poisson Process (NHPP), and Hawkes process \citep{hawkes1971spectra}---focus on modeling \emph{when} shocks occur. However, for risk management purposes, we also need to understand \emph{how large} these shocks are. The Compound Poisson Process (CPP) \citep{cont2004financial,ross2014introduction} addresses this by modeling both the timing and magnitude of shocks in a unified framework.
...
The CPP provides a principled framework for answering these questions \citep{cont2004financial}. In addition, we model the \emph{continuous} component of volatility dynamics using a mean--variance decomposition on $d_t=\Delta\log(\mathrm{VIX})_t$: an ARIMA model \citep{box2015time} for the conditional mean and GARCH-family models \citep{bollerslev1986generalized,nelson1991conditional} (GARCH/EGARCH) for the conditional variance. This volatility modeling complements the CPP shock framework by capturing volatility clustering and asymmetry, and it enables a concise persistence metric via the \textbf{volatility half-life} (how quickly volatility shocks decay back toward baseline).

% ============================================================
\section{Foundations}
% ============================================================

This section provides foundational context and mathematical definitions used throughout the report: (i) data transformation and basic time-series properties for VIX, (ii) volatility-model foundations (ARIMA and GARCH-family), and (iii) the Compound Poisson Process foundations for shock modeling.

% ------------------------------------------------------------
\subsection{Data transformation, autocorrelation, and stationarity}
% ------------------------------------------------------------
...
\subsubsection{VIX level versus SPY (context)}
...
The VIX level exhibits strong persistence (slowly decaying ACF), while $\Delta \log(\mathrm{VIX})$ is much closer to a weakly dependent / near white-noise series. Motivated by this difference, our ARIMA mean model (selected by BIC) is fit to $d_t=\Delta\log(\mathrm{VIX})_t$ (i.e., log returns of VIX), and the GARCH/EGARCH volatility models are then fit to the resulting ARIMA residuals.
...
We apply both ADF (unit-root null) \citep{dickey1979distribution} and KPSS (stationarity null) \citep{kwiatkowski1992testing} tests. The results show mixed evidence for the VIX \emph{level}, but consistent evidence that $\Delta \log(\mathrm{VIX})$ is stationary.
...
% ------------------------------------------------------------
\subsection{Mathematical foundations of volatility models}
% ------------------------------------------------------------

We decompose dynamics into a conditional mean model for $d_t=\Delta\log(\mathrm{VIX})_t$ and a conditional variance model for its innovations.

\subsubsection{ARIMA mean model}
Let $\varepsilon_t$ denote innovations. An ARIMA$(p,0,q)$ (ARMA$(p,q)$) model \citep{box2015time} takes the form
\[
d_t = c + \sum_{i=1}^{p}\phi_i d_{t-i} + \varepsilon_t + \sum_{j=1}^{q}\theta_j \varepsilon_{t-j}.
\]

\subsubsection{GARCH$(p,q)$ conditional variance}
A GARCH$(p,q)$ model \citep{bollerslev1986generalized} specifies time-varying conditional variance via
\[
\sigma_t^2 = \omega + \sum_{i=1}^{p}\alpha_i \varepsilon_{t-i}^2 + \sum_{j=1}^{q}\beta_j \sigma_{t-j}^2.
\]
For GARCH$(1,1)$, persistence is often summarized by $\alpha+\beta$ and the half-life is approximated by
\[
h_{1/2} \approx \frac{\log(0.5)}{\log(\alpha+\beta)}.
\]

\subsubsection{EGARCH$(p,q)$ conditional variance}
EGARCH models \citep{nelson1991conditional} log-variance and allow asymmetric responses. For EGARCH$(1,1)$, letting $z_{t-1}=\varepsilon_{t-1}/\sigma_{t-1}$,
\[
\log(\sigma_t^2)
=
\omega
...
\gamma\,z_{t-1}.
\]
A common half-life approximation uses the AR(1)-like coefficient on $\log(\sigma_t^2)$:
\[
h_{1/2} \approx \frac{\log(0.5)}{\log(\beta)}.
\]

% ------------------------------------------------------------
% Section 2.3 moved to Appendix
% ------------------------------------------------------------
...
This section summarizes the overall methodology (aligned with our presentation workflow): preprocessing and diagnostics, volatility model fitting and selection, shock extraction, jump-size distribution selection, and risk quantification via simulation and regime analysis.

% ------------------------------------------------------------
\subsection{Volatility modeling workflow and model selection}
% ------------------------------------------------------------

\subsubsection{Mean model selection (ARIMA)}
We model $d_t=\Delta\log(\mathrm{VIX})_t$ and select the ARIMA order by information criteria (BIC) \citep{schwarz1978estimating}. The chosen specification is ARIMA$(1,0,1)$ (ARMA$(1,1)$).

\subsubsection{Volatility model selection (GARCH/EGARCH)}
We fit GARCH-family models on the ARMA residuals and compare variants by AIC \citep{akaike1974new} and BIC \citep{schwarz1978estimating} across (i) volatility specification (GARCH vs.\ EGARCH), (ii) orders $(p,q)\in\{1,2\}\times\{1,2\}$, and (iii) innovation distribution (normal vs.\ Student-\emph{t}). Table~\ref{tab:garch_grid} reports the comparison (sorted by BIC).

\begin{table}[H]
\centering
\begin{tabular}{lccccc}
\toprule
Vol model & $p$ & $q$ & Dist & AIC & BIC \\
\midrule
EGARCH & 1 & 1 & t      & 26506.160007 & 26537.629005 \\
EGARCH & 2 & 1 & t      & 26505.758728 & 26543.521526 \\
EGARCH & 1 & 2 & t      & 26508.160007 & 26545.922804 \\
EGARCH & 2 & 2 & t      & 26507.758727 & 26551.815325 \\
GARCH  & 1 & 1 & t      & 26673.490706 & 26698.665904 \\
GARCH  & 2 & 1 & t      & 26673.867460 & 26705.336458 \\
GARCH  & 1 & 2 & t      & 26675.490706 & 26706.959704 \\
GARCH  & 2 & 2 & t      & 26674.752678 & 26712.515476 \\
EGARCH & 1 & 1 & normal & 26916.015508 & 26941.190706 \\
EGARCH & 2 & 1 & normal & 26917.590890 & 26949.059888 \\
EGARCH & 1 & 2 & normal & 26918.015507 & 26949.484506 \\
EGARCH & 2 & 2 & normal & 26919.590890 & 26957.353688 \\
GARCH  & 1 & 1 & normal & 27220.738722 & 27239.620121 \\
GARCH  & 2 & 1 & normal & 27222.470218 & 27247.645417 \\
GARCH  & 1 & 2 & normal & 27222.738722 & 27247.913920 \\
GARCH  & 2 & 2 & normal & 27223.425748 & 27254.894746 \\
\bottomrule
\end{tabular}
\caption{GARCH/EGARCH model comparison on ARMA residuals (sorted by BIC).}
\label{tab:garch_grid}
\end{table}

% ------------------------------------------------------------
\subsection{Shock modeling workflow (CPP)}
% ------------------------------------------------------------
...
% Section 3.2.1 moved to Appendix

\subsubsection{Distribution selection methodology}
...
To compare models with different numbers of parameters \citep{akaike1974new}:
\begin{equation}
...
The KS statistic \citep{kolmogorov1933sulla,smirnov1948table} measures the maximum discrepancy between empirical and fitted CDFs:
\begin{equation}
...
The \textbf{Pareto distribution} \citep{embrechts1997modelling} with $\alpha = 2.50$ and $x_{\min} = 0.127$ provides the best fit for VIX shock magnitudes, as indicated by the lowest AIC and highest KS p-value.

% ============================================================
\section{Volatility Model Fitting Results}
% ============================================================

\subsection{Mean dynamics: best ARIMA for $\Delta \log(\mathrm{VIX})$ (by BIC)}
Let $d_t = \Delta \log(\mathrm{VIX})_t$. The best mean specification selected by BIC \citep{schwarz1978estimating} is an ARIMA$(1,0,1)$ (i.e., ARMA$(1,1)$) model \citep{box2015time}:
\[
...
\subsection{Volatility dynamics: best GARCH-family specification (by BIC)}
We fit GARCH-family models \citep{bollerslev1986generalized,nelson1991conditional} to the ARMA residuals $\varepsilon_t$ with both normal and Student-\emph{t} innovations. By BIC \citep{schwarz1978estimating}, the best volatility specification is \textbf{EGARCH$(1,1)$ with Student-\emph{t} innovations} \citep{nelson1991conditional}.
...
\subsection{Volatility persistence: half-life}
...
\subsection{Conditional volatility overlays}
...
The Value-at-Risk at confidence level $\alpha$ for the annual shock impact is \citep{jorion2007value}:
\begin{equation}
...
\begin{remark}
CVaR is a \textbf{coherent risk measure} \citep{artzner1999coherent}, satisfying subadditivity: $\text{CVaR}(X + Y) \leq \text{CVaR}(X) + \text{CVaR}(Y)$. VaR does not satisfy this property.
\end{remark}
...
\begin{enumerate}
    ...
    \item \textbf{Out-of-Sample Performance}: The CPP model demonstrates reliable forecasting ability:
    \begin{itemize}
        \item Forecast errors of $\sim$18\% on the 2022--2025 test period
        \item VaR bounds were \textbf{not exceeded}, confirming conservative risk estimates
        \item Model is \textbf{well-calibrated}---actual outcomes fall within the bulk of predicted distributions
    \end{itemize}
...
The mathematical elegance of the CPP---combined with its empirical out-of-sample validity and practical applicability---makes it an essential tool for volatility risk management. In parallel, ARIMA + GARCH-family models provide a complementary view of \emph{continuous} volatility dynamics: model selection favors EGARCH$(1,1)$ with Student-\emph{t} innovations, and estimated half-lives indicate that volatility shocks decay over roughly one trading week.

% ============================================================
\section*{Appendix}
% ============================================================

\subsection{Mathematical foundations of the Compound Poisson Process}

\subsubsection{Definition of the Compound Poisson Process}

\begin{definition}[Compound Poisson Process]
A \textbf{Compound Poisson Process} $\{S(t) : t \geq 0\}$ \citep{cont2004financial,ross2014introduction} is defined as:
\begin{equation}
...
\subsubsection{Interpretation for VIX Shocks}
...
\paragraph{Mean of the Compound Poisson Process}
...
\paragraph{Variance of the Compound Poisson Process}
...
\subsection{Jump size distributions}
...
\begin{itemize}
    ...
    \item Common in financial applications for extreme events \citep{embrechts1997modelling}
\end{itemize}

\begin{remark}
The Pareto distribution is characterized by the \textbf{tail index} $\alpha$. Lower $\alpha$ means heavier tails (more extreme events). For financial data, $\alpha \in [2, 4]$ is typical \citep{embrechts1997modelling}.
\end{remark}
